...
var contract = new web3.eth.Contract(VND.abi, process.env.CONTRACT_ADDRESS, {
...
const service = {
  ...
  buy: async function (
    ...
    var tmpContract = new web3.eth.Contract(
      VND.abi,
      process.env.CONTRACT_ADDRESS,
      { from: web3.eth.accounts.wallet[0].address, gas: 3000000 }
    );
    web3.eth.defaultAccount = web3.eth.accounts.wallet[0].address;

    const data = await tmpContract.methods
      .buy(film, cinema, room, position, timestamp, fee)
      .send();
  ...
  cancel: async function (
    ...
    web3.eth.accounts.wallet.add(privateKey);
    const data = await contract.methods
      .cancel(film, cluster, cinema, room, position, timestamp)
      .send();
    web3.eth.accounts.wallet.clear();
    web3.eth.accounts.wallet.add(process.env.OWNER_PRIVATE_KEY);
    return data;
  ...
  chargeHistory: async function (owner) {
    //Lịch sử nạp tiền
    const histories = await contract.getPastEvents("Transfer", {
      fromBlock: 0,
      toBlock: "latest",
    });
    var results = [];
  ...
  buyHistory: async function (owner) {
    //Lịch sử mua vé
    const histories = await contract.getPastEvents("Buy", {
      fromBlock: 0,
      toBlock: "latest",
    });
    var results = [];
  ...
  cancelHistory: async function (owner) {
    //Lịch sử hủy vé
    const histories = await contract.getPastEvents("Cancel", {
      fromBlock: 0,
      toBlock: "latest",
    });
    var results = [];
